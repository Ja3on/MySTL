...
namespace MySTL
{
	...
	{
	public:
		using array_type = MySTL::vector<int>;
	private:
	...
	private:
		template <typename InputIterator>
		bool cmp(const InputIterator arr, size_t a, size_t b, size_t l);
		void calRank()
		{
		...
		template <typename InputIterator>
		void calHeight(const InputIterator arr, size_t len);
	};
	template <typename InputIterator>
	bool suffix_array::cmp(const InputIterator arr, size_t a, size_t b, size_t l)
	{
		return ((arr[a] == arr[b]) && (arr[a + l] == arr[b + l]));
	}
	template <typename InputIterator>
	void suffix_array::calHeight(const InputIterator arr, size_t len)
	{
	...
	void suffix_array::calSuffix(const InputIterator arr, size_t len, size_t max_len)
	{
		...
		//因为后续只是涉及x[]的比较工作，因而这一步可以不用存储真实的rank值，能够反映相对的大小即可。
